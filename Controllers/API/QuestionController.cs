using Microsoft.AspNetCore.Mvc;
using Prexam.Services;
using Prexam.Models;
using Prexam.DTOs;
using System.Net;

namespace Prexam.Controllers.Api
{
    [ApiController]
    [Route("api/[controller]")]
    public class QuestionController(IService<Question, QuestionRequest> service) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> Index()
        {
            var exams = await service.GetAllAsync();
            return Ok(new ApiResponse<IEnumerable<Question>>(HttpStatusCode.OK, "Success", exams));
        }

        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(Guid id)
        {
            var exam = await service.GetByIdAsync(id);
            if (exam == null)
            {
                return NotFound(new ApiResponse<Question>(HttpStatusCode.NotFound, "Exam not found"));
            }
            return Ok(new ApiResponse<Question>(HttpStatusCode.OK, "Success create data", exam));
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] QuestionRequest request)
        {
            Question exam = service.GetEntity(request);
            var success = await service.AddAsync(exam);
            if (!success) return NotFound(new ApiResponse<Question>(HttpStatusCode.NotFound, "Collection not found"));
            return Created("", new ApiResponse<Question>(HttpStatusCode.Created, "Success", exam));
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] QuestionRequest request)
        {
            Question exam = service.GetEntity(request);
            var success = await service.UpdateAsync(id, exam);
            if (!success) return NotFound(new ApiResponse<Question>(HttpStatusCode.NotFound, "Exam not found"));
            return Ok(new ApiResponse<Question>(HttpStatusCode.OK, "Success update data", exam));
        }

        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete(Guid id)
        {
            var success = await service.DeleteAsync(id);
            if (!success) return NotFound(new ApiResponse<Question>(HttpStatusCode.NotFound, "Exam not found"));
            return Ok(new ApiResponse<Question>(HttpStatusCode.OK, "Success delete data"));
        }
    }
}